...
int arithmetic_hdr(unsigned char *dst_addr,uint32_t width,uint32_t height)
{
	...
	unsigned char *temp_addr0 = s_hdr_cxt->addr[0];
	unsigned char *temp_addr1 = s_hdr_cxt->addr[1];
	unsigned char *temp_addr2 = s_hdr_cxt->addr[2];
	char           *p_format = IMAGE_FORMAT;
#if 0
	FILE *fp = NULL;

	CMR_LOGV("hdr: savedata");
	fp = fopen("/data/out0.raw", "wb");
	if(0 != fp)
	{
		fwrite((void*)temp_addr0, 1, size, fp);
		fclose(fp);
	}else{

		CMR_LOGE("can not create savedata");
	}
	fp = fopen("/data/out1.raw", "wb");
	if(0 != fp)
	{
		fwrite((void*)temp_addr1, 1, size, fp);
		fclose(fp);
	}else{

		CMR_LOGE("can not create savedata");
	}
	fp = fopen("/data/out2.raw", "wb");
	if(0 != fp)
	{
		fwrite((void*)temp_addr2, 1, size, fp);
		fclose(fp);
	}else{
	...
	if ((NULL != temp_addr0) && (NULL != temp_addr1) && (NULL != temp_addr2)) {
		...
		free(temp_addr1);
		free(temp_addr2);
		s_hdr_cxt->addr[0] = NULL;
		s_hdr_cxt->addr[1] = NULL;
		s_hdr_cxt->addr[2] = NULL;
	} else {
...
void arithmetic_hdr_data(unsigned char *addr,uint32_t size,uint32_t cap_cnt)
{
	...
	if (NULL == s_hdr_cxt->addr[cap_cnt-1]) {
		s_hdr_cxt->addr[cap_cnt-1] = (unsigned char*)malloc(size);
	}

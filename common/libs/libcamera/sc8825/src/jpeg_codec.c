...
static int _decoder(JPEGDEC_INPUT_PARAMS_T  *input_param)
{
	...
	CMR_LOGV("decoder S.");
	CMR_LOGV("format %d, w h %d %d, src_size 0x%x src_virt_buf 0x%x src_phy_buf 0x%x",
		(uint32_t)input_param->format,
		input_param->width,
		input_param->height,
		input_param->src_size,
		(uint32_t)input_param->src_virt_buf,
		input_param->src_phy_buf);

	CMR_LOGV("target_virt_buf_Y 0x%x, target_phy_buf_Y 0x%x, target_virt_buf_UV 0x%x, target_phy_buf_UV 0x%x",
		(uint32_t)input_param->target_virt_buf_Y,
		input_param->target_phy_buf_Y,
		(uint32_t)input_param->target_virt_buf_UV,
		input_param->target_phy_buf_UV);
	...
	CMR_LOGV("camera_decode_one_pic:set_slice_height=%d.",jpegdec_params.set_slice_height);

	ret = JPEGDEC_decode_one_pic(&jpegdec_params,input_param->write_yuv_callback);
	free(jpegdec_params.fw_decode_buf);
	CMR_LOGV("decoder E: %d.",ret);
	return ret;
}
...
static int _dec_start0(uint32_t handle)
{
	int ret = JPEG_CODEC_SUCCESS;
	...
	if (dec_cxt_ptr->size.height >  dec_cxt_ptr->slice_height) {
		...
		input_param_ptr->target_phy_buf_UV = dec_cxt_ptr->temp_buf_phy+dec_cxt_ptr->slice_height*dec_cxt_ptr->size.width;
	...
	input_param_ptr->write_yuv_callback = _dec_callback;  //maybe need to add some thing.
...
static int _dec_start(uint32_t handle)
{
	...
	dec_cxt_ptr->cur_line_num = 0;
	dec_cxt_ptr->is_finish = 0;
	dec_cxt_ptr->fw_decode_buf_size = 20*1024;
	dec_cxt_ptr->fw_decode_buf = (void*)malloc(dec_cxt_ptr->fw_decode_buf_size);
	...
	if(dec_cxt_ptr->slice_height == dec_cxt_ptr->size.height) {
		jpegdec_params.set_slice_height = 128;
		dec_cxt_ptr->set_slice_height = 128;
	}

	memset(&slice_out,0,sizeof(JPEGDEC_SLICE_OUT_T));

	if(0 != JPEGDEC_Slice_Start(&jpegdec_params,  &slice_out)) {
		free(dec_cxt_ptr->fw_decode_buf);
		dec_cxt_ptr->fw_decode_buf = NULL;
		ret = JPEG_CODEC_ERROR;
	}
...
static int _jpeg_enc_wexif(struct jpeg_enc_exif_param *param_ptr,struct jpeg_wexif_cb_param *out_ptr)
{
	...
	if(NULL == input_param.temp_buf_ptr) {

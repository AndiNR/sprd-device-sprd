...
public class FmReceiveView extends FrameLayout implements View.OnClickListener,
	...
	private boolean allIsEnabled = true;
	...
	public void init(IRadioViewRxTouchEventHandler eventHandler) {
		...
		final int[][] buttonTranslationTable = {
				...
				{ R.id.radio_list, FmConstants.BUTTON_RADIO_LIST},
				{ R.id.add_radio, FmConstants.BUTTON_ADD_RADIO},
				{ R.id.auto_search, FmConstants.BUTTON_AUTO_SEARCH},
	...
	public void setEnabled(boolean enabled) {
		allIsEnabled = enabled;
		if(sliders!=null && !sliders.isEmpty()){
			for(Integer id:sliders){
...
//	@Override
    ...
    // this is a not very good way to avoid a problem that is happened in bsp
	// if this is enabled, we only accept the updating of unmute status
    boolean mSkipUpdatingViewForMuted = false;

    public void enableSkipUpdatingViewForMuted() {
        mSkipUpdatingViewForMuted = true;
    }

	/* Display muted/unmuted icon. */
	public synchronized void setMutedState(boolean isMute, boolean inCall) {

	    if (mSkipUpdatingViewForMuted && isMute)
	        return;

	    if (!isMute)
	        mSkipUpdatingViewForMuted = false;
		...
		ib.setEnabled(allIsEnabled==true?(!inCall):false);
	}
	...
	public synchronized void setSeekStatus(boolean seekInProgress, boolean tuneInProgress) {
		findViewById(R.id.seek_indicator).setVisibility(View.GONE/*seekInProgress || tuneInProgress ? View.VISIBLE : View.INVISIBLE*/);
		findViewById(FmConstants.BUTTON_TUNE_UP)  .setEnabled(allIsEnabled==true?(!seekInProgress):false);
		findViewById(FmConstants.BUTTON_TUNE_DOWN).setEnabled(allIsEnabled==true?(!seekInProgress):false);
		findViewById(FmConstants.BUTTON_SEEK_UP)  .setEnabled(allIsEnabled==true?(!seekInProgress):false);
		findViewById(FmConstants.BUTTON_SEEK_DOWN).setEnabled(allIsEnabled==true?(!seekInProgress):false);
	}
